...
const FALLBACK_EVENTS = [
  ...
  { id: 'f2', date: 'April 9, 2026', title: 'Got Bags Collection Drive', time: '10:00 AM – 2:00 PM', location: 'Milford Town Green', type: 'project' },
  { id: 'f3', date: 'April 22, 2026', title: 'Earth Day Cleanup', time: '9:00 AM – 12:00 PM', location: 'Eisenhower Park', type: 'volunteer' },
  { id: 'f4', date: 'May 7, 2026', title: 'End-of-Year Banquet', time: '6:00 PM – 8:30 PM', location: 'School Cafeteria', type: 'social' },
];
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {items.map((event, i) => {
                ...
                return (
                  <div key={event.id || i} className="bg-card rounded-xl border border-border p-5 md:p-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      <div className="shrink-0 w-16 h-16 rounded-lg bg-primary flex flex-col items-center justify-center text-primary-foreground">
                        <span className="text-xs font-medium uppercase">
                          {validDate ? parsed.toLocaleDateString('en-US', { month: 'short' }) : '—'}
                        </span>
                        <span className="text-xl font-bold leading-none">
                          {validDate ? parsed.getDate() : '?'}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {event.type && (
                            <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${typeColors[event.type] || 'bg-muted text-muted-foreground'}`}>
                              {event.type}
                            </span>
                          )}
                        </div>
